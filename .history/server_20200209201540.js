...
fs.readFile('client/slowa.txt', function(err, data, res) {
    ...
    console.log(array);
    array.prop1 = [];
    array.prop1.push("man");
    array.prop2 = {}; // <= Object instead of array
    array.prop2["hey"] = "man";
    // array.push({ roomName: "New Room", users: ["John", "Derek"] });  
});
...
io.on('connection', socket => {
  ...
  socket.on('send-word', (room, word, array) => {
    socket.to(room).broadcast.emit('word-message', word,{
    name: rooms[room].users[socket.id] })
    // proba wyslania tablicy slow
    socket.emit('return_rooms', array);
  })
